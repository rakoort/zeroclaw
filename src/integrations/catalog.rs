...
/// A registered integration entry for the CLI catalog.
...
fn show_integration_info(config: &Config, name: &str) -> Result<()> {
    let entries = catalog_registry::all_integrations();
    ...
    let (icon, label) = match status {
        IntegrationStatus::Active => ("\u{2705}", "Active"),
        IntegrationStatus::Available => ("\u{26aa}", "Available"),
        IntegrationStatus::ComingSoon => ("\u{1f51c}", "Coming Soon"),
    };
    ...
    match entry.name {
        ...
        "Discord" => {
            ...
            println!("    2. Create app \u{2192} Bot \u{2192} Copy token");
        ...
        "Slack" => {
            ...
            println!("    2. Create app \u{2192} Bot Token Scopes \u{2192} Install");
            println!("    3. Run: zeroclaw onboard --channels-only");
        }
        "OpenRouter" => {
            println!("  Setup:");
        ...
        "iMessage" => {
            ...
            println!("    Requires Full Disk Access in System Settings \u{2192} Privacy.");
        }
        "GitHub" => {
            println!("  Setup:");
...
mod tests {
    ...
    fn handle_command_info_is_case_insensitive_for_known_integrations() {
        ...
        let first_name = catalog_registry::all_integrations()
            .first()
