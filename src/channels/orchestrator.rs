//! Channel message dispatch and orchestration.
//!
//! Contains process_channel_message, run_message_dispatch_loop, start_channels,
//! and all supporting infrastructure for receiving, routing, and processing
//! channel messages.

use super::traits;
use super::{
    build_models_help_response, build_providers_help_response, collect_configured_channels,
    create_resilient_provider_nonblocking, get_or_create_provider, resolve_provider_alias,
    resolved_default_model, resolved_default_provider, runtime_defaults_from_config,
    ChannelRouteSelection, ChannelRuntimeCommand, ChannelRuntimeContext, ChannelRuntimeDefaults,
    ConfigFileStamp, InFlightSenderTaskState, InFlightTaskCompletion, RuntimeConfigState,
    AUTOSAVE_MIN_MESSAGE_CHARS, BOOTSTRAP_MAX_CHARS, CHANNEL_HEALTH_HEARTBEAT_SECS,
    CHANNEL_HISTORY_COMPACT_CONTENT_CHARS, CHANNEL_HISTORY_COMPACT_KEEP_MESSAGES,
    CHANNEL_HOOK_MAX_OUTBOUND_CHARS, CHANNEL_MAX_IN_FLIGHT_MESSAGES,
    CHANNEL_MESSAGE_TIMEOUT_SCALE_CAP, CHANNEL_MIN_IN_FLIGHT_MESSAGES,
...
#[cfg(test)]
use super::{
    classify_health_result, runtime_config_store, ChannelHealthState, CHANNEL_MESSAGE_TIMEOUT_SECS,
};
...
/// Build a conversation history key from a channel message.
///
/// Threaded messages are keyed by (channel, thread_ts) — NOT by sender —
/// so all participants in the same thread share one history.  Non-threaded
/// messages are keyed by (channel, sender) for per-user isolation.
pub fn conversation_history_key(msg: &traits::ChannelMessage) -> String {
    match &msg.thread_ts {
...
pub(crate) async fn maybe_apply_runtime_config_update(ctx: &ChannelRuntimeContext) -> Result<()> {
    let Some(config_path) = super::runtime_config_path(ctx) else {
        return Ok(());
    };
    ...
    {
        let store = super::runtime_config_store()
            .lock()
    ...
    {
        let mut store = super::runtime_config_store()
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        store.insert(
            config_path.clone(),
...
pub(crate) async fn process_channel_message(
    ...
    let runtime_defaults = super::runtime_defaults_snapshot(ctx.as_ref());
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    let provider_runtime_options = providers::ProviderRuntimeOptions {
        ...
        tool_call_mode: config.runtime.tool_call_mode.clone(),
    };
    ...
    let runtime_ctx = Arc::new(ChannelRuntimeContext {
        ...
        max_executor_action_iterations: config.agent.max_executor_action_iterations,
...
mod tests {
    ...
    fn compact_sender_history_keeps_recent_truncated_messages() {
        ...
        let ctx = ChannelRuntimeContext {
            ...
            max_executor_action_iterations: 15,
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    async fn message_dispatch_processes_messages_in_parallel() {
        ...
        assert!(
            ...
            "expected parallel dispatch (<500ms), got {:?}",
            elapsed
        );
