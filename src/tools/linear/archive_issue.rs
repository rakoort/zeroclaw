...
impl Tool for LinearArchiveIssueTool {
    ...
    fn parameters_schema(&self) -> serde_json::Value {
        serde_json::json!({
            "type": "object",
            "properties": {
                "issue_id": { "type": "string", "description": "Linear issue ID to archive" },
...
mod tests {
    ...
    #[test]
    fn linear_archive_issue_tool_metadata_and_required_params() {
        let config = Arc::new(LinearToolConfig::new(
            "linear-cli.ts",
            Path::new("/workspace"),
        ));
        let tool = LinearArchiveIssueTool { config };

        assert_eq!(tool.name(), "linear_archive_issue");
