//! HTTP client factory with runtime proxy support.
//!
//! This module manages proxy-aware `reqwest::Client` construction for all
//! outbound HTTP traffic in ZeroClaw. Clients are cached by service key and
//! timeout parameters so repeated callers reuse the same connection pool.
...
mod tests {
    ...
    fn build_runtime_proxy_client_returns_usable_client() {
        let client = build_runtime_proxy_client("provider.test_common_http");
        drop(client);
    }
    ...
    fn build_runtime_proxy_client_with_timeouts_returns_usable_client() {
        let client =
            build_runtime_proxy_client_with_timeouts("provider.test_common_http_timeout", 30, 5);
        drop(client);
    }

    #[test]
    fn cache_key_includes_service_and_timeouts() {
        let key = runtime_proxy_cache_key("provider.openai", Some(120), Some(10));
        assert_eq!(key, "provider.openai|timeout=120|connect_timeout=10");

        let key_none = runtime_proxy_cache_key("provider.openai", None, None);
        assert_eq!(
            key_none,
            "provider.openai|timeout=none|connect_timeout=none"
        );
    }

    #[test]
    fn cache_key_normalizes_service_key_case() {
        let key = runtime_proxy_cache_key("Provider.OpenAI", None, None);
        assert_eq!(key, "provider.openai|timeout=none|connect_timeout=none");
    }

    #[test]
    fn normalize_proxy_url_option_trims_whitespace() {
        assert_eq!(
    ...
    #[test]
    fn normalize_proxy_url_option_returns_none_for_empty() {
    ...
    #[test]
    fn normalize_service_list_deduplicates_and_lowercases() {
        let input = vec![
            "Provider.OpenAI, provider.openai".to_string(),
            "channel.discord".to_string(),
        ];
        let result = normalize_service_list(input);
        assert_eq!(result, vec!["channel.discord", "provider.openai"]);
    }

    #[test]
    fn parse_proxy_scope_recognizes_aliases() {
        assert_eq!(
            parse_proxy_scope("environment"),
            Some(ProxyScope::Environment)
        );
        assert_eq!(parse_proxy_scope("env"), Some(ProxyScope::Environment));
        assert_eq!(parse_proxy_scope("zeroclaw"), Some(ProxyScope::Zeroclaw));
        assert_eq!(parse_proxy_scope("internal"), Some(ProxyScope::Zeroclaw));
        ...
        assert_eq!(parse_proxy_scope("bogus"), None);
    }

    #[test]
    fn parse_proxy_enabled_recognizes_truthy_and_falsy() {
        assert_eq!(parse_proxy_enabled("true"), Some(true));
        assert_eq!(parse_proxy_enabled("1"), Some(true));
        assert_eq!(parse_proxy_enabled("yes"), Some(true));
        assert_eq!(parse_proxy_enabled("on"), Some(true));
        assert_eq!(parse_proxy_enabled("false"), Some(false));
        assert_eq!(parse_proxy_enabled("0"), Some(false));
        assert_eq!(parse_proxy_enabled("no"), Some(false));
        assert_eq!(parse_proxy_enabled("off"), Some(false));
        assert_eq!(parse_proxy_enabled("maybe"), None);
    }
    ...
    fn service_selector_matches_exact_and_wildcard() {
        ...
        assert!(service_selector_matches("provider.*", "provider.openai"));
        assert!(service_selector_matches("provider.*", "provider.anthropic"));
        assert!(!service_selector_matches("provider.*", "channel.discord"));
    }

    #[test]
    fn proxy_config_should_apply_disabled_returns_false() {
        let config = ProxyConfig::default();
        assert!(!config.should_apply_to_service("provider.openai"));
    }

    #[test]
    fn proxy_config_should_apply_zeroclaw_scope_returns_true() {
        ...
        assert!(config.should_apply_to_service("provider.openai"));
        assert!(config.should_apply_to_service("channel.discord"));
    }

    #[test]
    fn proxy_config_should_apply_services_scope_filters() {
        let config = ProxyConfig {
            enabled: true,
            http_proxy: Some("http://proxy:8080".into()),
    ...
    #[test]
    fn proxy_config_validate_rejects_missing_url_when_enabled() {
        let config = ProxyConfig {
            enabled: true,
            ..ProxyConfig::default()
        };
        let err = config.validate().unwrap_err().to_string();
        assert!(err.contains("no proxy URL is configured"));
    }

    #[test]
    fn proxy_config_validate_rejects_empty_services_with_services_scope() {
        let config = ProxyConfig {
            enabled: true,
            http_proxy: Some("http://proxy:8080".into()),
            scope: ProxyScope::Services,
    ...
    #[test]
    fn set_runtime_proxy_config_clears_cache() {
        let service_key = format!(
            "provider.common_http_cache_test.{}",
        ...
        assert!(runtime_proxy_cached_client(&cache_key).is_some());

        set_runtime_proxy_config(ProxyConfig::default());
        assert!(runtime_proxy_cached_client(&cache_key).is_none());
    }
}
